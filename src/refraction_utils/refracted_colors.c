...
t_vec3	refracted_color(t_world *world, t_computation *comps, int depth)
{
	...
	direction = vector_sub(
			vector_scale(comps->normalv, (n_ratio * cos_i) - cos_t),
			vector_scale(comps->eyev, n_ratio)
			);
	refracted_ray = ray_constructor(comps->under_point, direction);
	return (vector_scale(
				color_at(world, &refracted_ray, depth - 1),
...
float	schlick(t_computation *comps)
{
	...
	return (r0 + (1.0f - r0) * (inv_cos * inv_cos * inv_cos * inv_cos * inv_cos));
}
